...
void lcd_set_direction(lcd_dir_t dir)
{
    ...
    if (dir & DIR_XY_MASK)
    {
        lcd_ctl.width = CONFIG_LCD_WIDTH - 1;
        lcd_ctl.height = CONFIG_LCD_HEIGHT - 1;
    }
    else
    {
        lcd_ctl.width = CONFIG_LCD_HEIGHT - 1;
        lcd_ctl.height = CONFIG_LCD_WIDTH - 1;
    }
...
void lcd_clear(uint16_t color)
{
    ...
    tft_fill_data(&data, CONFIG_LCD_HEIGHT * CONFIG_LCD_WIDTH / 2);
}
